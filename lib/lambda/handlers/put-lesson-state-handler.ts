...
import { PutLessonStateUseCase } from '../domain/use-cases/LessonStateUseCases';
import { Logger } from '../shared/logger';
import { DomainError } from '../shared/errors';

const logger = new Logger('PutLessonStateHandler');
const tableName = process.env.TABLE_NAME!;

export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  logger.info('Put lesson state request received', {
  ...
  try {
    ...
    const useCase = new PutLessonStateUseCase(repository);

    await useCase.execute(state);

    logger.info('Lesson state saved successfully', { userId, lessonId });
